...
import { quizApi, adminApi } from '../../../core/utils/api'
...
const AdminQuiz = () => {
  ...
  const fetchQuizzes = async (page: number = 1, search: string = '', courseId: string = '') => {
    try {
      setLoading(true)
      const response = await adminApi.getAdminQuizzes({
        page: page.toString(),
        limit: '10',
        search,
        courseId
                      ...
                      <div key={quiz.id} className="border rounded-2 p-3 mb-3">
                        <div className="row align-items-center">
                          <div className="col-md-8">
                            <div>
                              <h6 className="mb-2">
                                <Link to={all_routes.adminQA.replace(':quizId',quiz.id)}>
                                  {quiz.title}
                                </Link>
                                {!quiz.isActive && (
                          ...
                          <div className="col-md-4">
                            <div className="d-flex align-items-center justify-content-end mt-2 mt-md-0">
                              <Link 
                                to={`${all_routes.adminQuizResult.replace(':quizId', quiz.id)}`} 
