...
import { adminApi, courseApi } from '../../../core/utils/api';
...
const AdminCourse = () => {
  ...
  const [courseToPublish, setCourseToPublish] = useState<string>('');
  const [courseToUnpublish, setCourseToUnpublish] = useState<string>('');
  ...
  const handleRequestChanges = async (courseId: string) => {
    try {
      const data = { status: 'submitted' }; // Changed from 'publish' to 'submitted'
      const response = await adminApi.publishCourse(courseId, data);
      ...
      if (response.success) {
        ...
        toast.success('Course status changed to Submitted');
      }
    } catch (error) {
      toast.error('Failed to submit course for Submitted');
    }
  };
  ...
  const handlePublishCourse = async () => {
    try {
      ...
      const response = await adminApi.publishCourse(courseToPublish, data);
      
      if (response.success) {
        setCourses(prevCourses => 
          prevCourses.map(course => 
            course.id === courseToPublish 
  ...
  const handleUnpublishCourse = async () => {
    try {
      ...
      const response = await adminApi.publishCourse(courseToUnpublish, data);
      
      if (response.success) {
        setCourses(prevCourses => 
          prevCourses.map(course => 
            course.id === courseToUnpublish 
              ? { ...course, status: 'draft' }
              : course
          )
        );
      ...
      setCourseToUnpublish('');
    }
  };

  const openPublishModal = (courseId: string, action: 'publish' | 'unpublish') => {
    if (action === 'publish') {
      setCourseToPublish(courseId);
      setPublishAction('publish');
    } else {
      setCourseToUnpublish(courseId);
  ...
  const columns = [
    {
      ...
      render: (text: string, record: any) => (
        <div className="d-flex align-items-center">
          <Link
            to={all_routes.adminCourseDetails.replace(":courseSlug", record.course.slug)}
          ...
          <div>
            <h6 className="fw-medium mb-2">
              <Link to={all_routes.adminCourseDetails.replace(":courseSlug", record.course.slug)}>
                {text}
              </Link>
            </h6>
            <div className="d-flex align-items-center">
              ...
              <span className="d-inline-flex fs-12 align-items-center">
                ...
                {record.course.stats.totalDuration ? formatDuration(record.course.stats.totalDuration) : '00:00:00'}
              </span>
            </div>
          </div>
        </div>
    ...
    },
    {
      ...
      render: (text: string) => (
        <span
          className={`badge badge-sm bg-skyblue d-inline-flex align-items-center me-1`}>
        ...
        <div className="d-flex align-items-center">
          ...
          <Link
            to={record.course.status === 'submitted' || record.course.status === 'approved' || record.course.status === 'published' 
              ? '#' 
              : `/admin/create-course/${record.course.id}`}
            className={`d-inline-flex fs-14 me-1 action-icon ${
              (record.course.status === 'submitted' || record.course.status === 'approved' || record.course.status === 'published' ) 
                ? 'opacity-50' 
                : ''
            }`}
            title={
          ...
          {record.course.status === 'approved' && (
            <Link
              to="#"
              className="d-inline-flex fs-14 me-1 action-icon text-success"
              data-bs-toggle="modal"
              data-bs-target="#publish_modal"
              onClick={(e) => {
                e.preventDefault();
                openPublishModal(record.course.id, 'publish');
              }}
              title="Publish Course"
            >
              <i className="isax isax-arrow-circle-up" />
            </Link>
          )}

          {/* If it's an instructor's course */}
          {record.course.courseType === 'instructor' && (
            <>
              {record.course.status !== 'published' ? (
                <Link
                  to="#"
                  className="d-inline-flex fs-14 me-1 action-icon text-secondary"
                  data-bs-toggle="modal"
                  data-bs-target="#publish_modal"
                  onClick={(e) => {
                    e.preventDefault();
                ...
                <Link
                  ...
                  className="d-inline-flex fs-14 me-1 action-icon text-secondary"
                  onClick={() => {
                    handleRequestChanges(record.key);
                  }}
                  title="Unpublish Course"
                >
                  <i className="isax isax-arrow-circle-down" />
                </Link>
              )}
            </>
          )}

           {record.course.courseType === 'admin' && (
            <>
              {record.course.status !== 'published' ? (
                <Link
                  to="#"
                ...
                <Link
                  ...
                  onClick={(e) => {
                    e.preventDefault();
                    openPublishModal(record.course.id, 'unpublish');
                  }}
                  title="Unpublish Course"
                >
                  <i className="isax isax-arrow-circle-down" />
          ...
          {record.course.status === 'submitted' && (
            <Link
              to="#"
              className="d-inline-flex fs-14 me-1 action-icon text-primary"
              onClick={(e) => {
                e.preventDefault();
                openPublishModal(record.course.id, 'publish');
              }}
            ...
            <div className="col-lg-9">
              ...
              <div className="page-title d-flex align-items-center justify-content-between">
                ...
                <div className="d-flex align-items-center list-icons">
                  ...
                  <Link to={all_routes.adminCourseGrid}>
      ...
      <>
        ...
        <div className="modal fade" id="publish_modal">
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              <div className="modal-body text-center custom-modal-body">
                <span className={`avatar avatar-lg rounded-circle mb-2 ${
                  publishAction === 'publish' ? 'bg-success-transparent' : 'bg-warning-transparent'
                }`}>
                  <i className={`isax fs-24 ${
                    publishAction === 'publish' ? 'isax-arrow-circle-up text-success' : 'isax-arrow-circle-down text-warning'
                  }`} />
                </span>
                <div>
                  <h4 className="mb-2">
                  ...
                  <div className="d-flex align-items-center justify-content-center">
                    ...
                    <Link
                      ...
                      className={`btn rounded-pill ${
                        publishAction === 'publish' ? 'btn-success' : 'btn-warning'
                      }`}
                      data-bs-dismiss="modal"
                      onClick={publishAction === 'publish' ? handlePublishCourse : handleUnpublishCourse}
                    >
                      {publishAction === 'publish' ? 'Yes, Publish' : 'Yes, Unpublish'}
                    </Link>
                  </div>
                </div>
              </div>
