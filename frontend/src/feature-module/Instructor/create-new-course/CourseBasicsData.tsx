...
const CourseBasics = () => {
  ...
  const saveDraft = async () => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (courseId === 'new') {
          navigate(`/instructor/create-course/${newCourseId}`, { replace: true });
        }
        setHasUnsavedChanges(false);
        return newCourseId;
      } else {
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      navigate(`/instructor/course-syllabus/${savedCourseId}`);
    } catch (error) {
  ...
  const handleCategoryChange = (value: string) => {
    setFormData(prev => ({
      ...prev,
      categoryId: value,
      specialtyId: '' // Reset specialty when category changes
    }));
    setHasUnsavedChanges(true);
